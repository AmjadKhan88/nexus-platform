...
export const InvestorsPage: React.FC = () => {
  ...
  const filteredInvestors = investors.filter((investor) => {
    const matchesSearch =
      ...
      investor.investmentInterests?.some((int: string) =>
        int.toLowerCase().includes(searchQuery.toLowerCase())
      );
    const matchesStages =
      !selectedStages.length ||
      investor.investmentStage?.some((s: string) => selectedStages.includes(s));
    const matchesInterests =
      !selectedInterests.length ||
      investor.investmentInterests?.some((i: string) => selectedInterests.includes(i));
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {loading ? (
            <p className="text-center py-12 text-gray-500">Loading investors...</p>
          ) : (
