...
export const CollaborationRequestCard: React.FC<CollaborationRequestCardProps> = ({
  ...
  // Support both populated object and plain id
  ...
  const handleAccept = () => onStatusUpdate?.(request._id, 'accepted');
  const handleReject = () => onStatusUpdate?.(request._id, 'rejected');
  const handleMessage = () => navigate(`/chat/${investorId}`);
  const handleViewProfile = () => navigate(`/profile/investor/${investorId}`);
